...
function App() {
    ...
    // useEffect(() => {
    //     setStage(Number(localStorage.getItem('todoList')) || 0)
    // }, [])
    useEffect(() => {
    ...
    return (
        <div className={`App position-relative`}>
            ...
            {stage < 99 && <main className={'p-2'}>
                ...
                <div className={''}>
                    {`счетчик неправильных ответов: ${count}`}
                </div>
                <div className={'d-flex justify-content-center'}>
                    <input className={'form-control w-75 me-2'} value={valueInput}
                           onChange={(e) => setValueInput(e.target.value)}/>
                    <button className={'btn btn-primary'} onClick={() => checkResult(valueInput)}>Ок</button>
